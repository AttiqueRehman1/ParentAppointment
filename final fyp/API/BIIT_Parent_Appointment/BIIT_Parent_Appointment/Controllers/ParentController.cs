...
namespace BIIT_Parent_Appointment.Controllers
{
    public class ParentController : ApiController
    {
        ...
        public HttpResponseMessage CountNotification(string id)
        {
            try
            {
                var notification = db.Meetings.Where(b => b.parentId == id&&b.studentMeeting==false);
        ...
        {//  /api/Parent/GetNotification?pcnic=3240277931065
            try
            {
                var meeting = db.Meetings.Where(m => m.parentId== pcnic&&m.studentMeeting==false).ToList();
        ...
        [HttpGet]
        public HttpResponseMessage GetIssueListObject()
        {
            try
            {
                var issueslist = db.IssuesLists.ToList();
                if (issueslist.Count != 0)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, issueslist);
                }
